...
export function getGitDiff(folder) {
  try {
    ...
    // Discover untracked files (new files never staged)
    try {
      ...
      if (untrackedOutput) {
        ...
        for (const relPath of untrackedOutput.split('\n')) {
          ...
          try {
            ...
            const hunkLines = contentLines.map(line => ({ type: 'addition', content: line }));
            ...
            parsed.stats.additions += additions;
          } catch (_) {
            // Skip files we can't read
          }
        }
        parsed.stats.files = parsed.files.length;
      }
    } catch (_) {
      // Ignore errors in untracked file discovery
    }
