...
import { getCodeServerPort, getAnyActiveCodeServerPort } from './code-server.mjs';
...
function getCodeServerPortFromCookie(req) {
  ...
  if (!folder) return null;
  return getCodeServerPort(folder) || getAnyActiveCodeServerPort();
}
...
export function handleWebSocketUpgrade(req, socket, head) {
  ...
  const upstream = net.connect(port, 'localhost', () => {
    let httpReq = `${req.method} ${forwardUrl} HTTP/${req.httpVersion}\r\n`;
