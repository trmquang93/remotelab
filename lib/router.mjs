...
import { loadSessions, saveSessions, generateId, sessionExists, killSession, getSessionSocketName, getSessionsByFolder } from './sessions.mjs';
import { getAvailableTools, addTool, removeTool, isToolValid } from './tools.mjs';
import { getGitDiff } from './git-diff.mjs';
import { loginPage, dashboardPage, sessionViewPage, folderViewPage } from './templates.mjs';
...
export async function handleRequest(req, res) {
  ...
  if (pathname.startsWith('/session/')) {
    ...
    const toolId = session.tool || 'claude';
    const html = sessionViewPage
      ...
      .replace(/\{\{SESSION_TOOL_ID\}\}/g, escapeHtml(toolId))
      .replace(/\{\{SESSION_TOOL_ID_URL\}\}/g, encodeURIComponent(toolId))
      .replace(/\{\{SESSION_TOOL_COMMAND_URL\}\}/g, encodeURIComponent(toolId));
