...
// even when this process is launched by launchd/systemd with a minimal PATH.
...
// Ensure the Node.js binary used to run this process is also discoverable
// for child processes (e.g. CLI tools installed in ~/.local/bin that use
// "/usr/bin/env node" in their shebang). This makes sure that even when
// the systemd/launchd PATH is minimal, tools still see the correct node.
try {
  const nodeDir = dirname(process.execPath || '');
  if (nodeDir && !fullPath.split(':').includes(nodeDir)) {
    fullPath = `${nodeDir}:${fullPath}`;
  }
} catch {
  // best-effort only; ignore if anything goes wrong
}

// Expose the resolved PATH so that other modules (e.g. session/ttyd launcher)
// can ensure child processes see the same tool search path that we use here
// when probing for tool availability.
export const TOOL_PATH = fullPath;
