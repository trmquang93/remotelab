...
trap 'status=$?; echo; echo "[smoke] failed with exit $status"; [ -f /tmp/remotelab-setup.log ] && { echo "--- setup log ---"; cat /tmp/remotelab-setup.log; }; echo "--- auth-proxy unit ---"; systemctl --user status remotelab-auth-proxy.service --no-pager || true; exit $status' ERR
...
log "Installing claude and ttyd test stubs"
...
wait_for_http "http://localhost:7681/login"
curl -fsS http://localhost:7681/login | grep -q "Claude Code"

log "Logging in and checking dashboard"
COOKIE_JAR=/tmp/remotelab-cookies.txt
curl -sS -c "$COOKIE_JAR" -b "$COOKIE_JAR" -o /tmp/login-response.html -D /tmp/login-headers.txt \
  -X POST http://localhost:7681/login \
  -H 'Content-Type: application/x-www-form-urlencoded' \
  --data-urlencode "username=claude" \
  --data-urlencode "password=$PASSWORD"
grep -q '^Location: /' /tmp/login-headers.txt
curl -fsS -c "$COOKIE_JAR" -b "$COOKIE_JAR" http://localhost:7681/ | grep -q "Session Manager"
...
CREATE_RESPONSE="$(curl -fsS -c "$COOKIE_JAR" -b "$COOKIE_JAR" \
  ...
  http://localhost:7681/api/sessions)"
...
curl -fsS -c "$COOKIE_JAR" -b "$COOKIE_JAR" http://localhost:7681/api/sessions | jq -e --arg id "$SESSION_ID" '.sessions[] | select(.id == $id)' >/dev/null
